...
module.exports = class DigiComm{
	...
	dataIn(d){
		...
		if(this.at_wait){
			...
			this.at_wait_resp+=String.fromCharCode(d);
			this.temp.push(d);
			var data = this.temp;
			//console.log(data);
			var cmd = String.fromCharCode(data[0])+String.fromCharCode(data[1])+String.fromCharCode(data[2]);
			if("OK\r" == cmd){
				var frame = {
					command: cmd
				}
	...
	persist_settings(){
		...
		return new Promise((fulfill, reject) => {
			setTimeout(() => {
				that.send_at_raw('+++').then((r) => {
					that.send_at_raw("ATWR\r").then((r) => {
						that.send_at_raw("ATCN\r").then((r) => {
							fulfill();
						}).catch(reject);
					}).catch(reject);
